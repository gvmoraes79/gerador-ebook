...
// Helper to get AI instance dynamically with the User's Key
...
export const generateChapterContent = async (apiKey: string, ebookTitle: string, chapterTitle: string, language: string, includeImages: boolean, observations: string): Promise<{ content: string; sources: string[]; image?: string }> => {
    const ai = getAi(apiKey);
    try {
...
export const structureText = async (apiKey: string, fullText: string): Promise<StructuredText> => {
    const ai = getAi(apiKey);
    try {
        const response = await ai.models.generateContent({
